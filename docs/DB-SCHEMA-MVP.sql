-- VectiScan MVP — Datenbankschema
-- Ersetzt das Prototyp-Schema (scans → orders, neue Tabellen)

-- ============================================================================
-- Status-Maschine (orders.status):
--
--   created → verification_pending → verified → payment_pending → paid
--   → scan_queued → dns_recon → scan_phase1 → scan_phase2 → scan_complete
--   → report_generating → report_complete → delivered
--   → failed (von jedem Status aus)
-- ============================================================================
...
-- Aufträge (ersetzt die alte scans-Tabelle)
CREATE TABLE orders (
    id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
