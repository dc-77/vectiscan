...
Stand 2026-05-04: ~25 Templates (deckt ~70% der haeufigsten Findings).
Long-Tail: weitere policy_ids iterativ ergaenzen.
"""
...
def apply_title_template(finding: dict[str, Any], scan_context: dict[str, Any] | None = None) -> str:
    ...
    Returns:
        ...
        Setzt finding['_title_template_missing'] = True bei Miss (fuer QA).
    """
    ...
    # Fallback-Vars aus haeufigen Finding-Feldern
    ...
    if "domain" not in title_vars and "host" in title_vars:
        # Domain aus Host extrahieren (best effort)
    ...
    try:
        return template.format_map(_SafeDict(title_vars))
