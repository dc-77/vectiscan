...
def _take_screenshot(
    ...
    """Take a viewport screenshot per VHost and return the path, or None on failure.

    Mai 2026: zurueck auf Viewport-Only (full_page=False). F-PH1-003 hatte
    full_page=True eingefuehrt — fuer Customer-Reports zu lang ("erste Bildschirm-
    Ansicht zeigen, nicht alles was man scrollen muesste"). Pro VHost separates
    File unter ``screenshots/<order_id_dir>/<ip>__<vhost>.png`` (Layout siehe
    upload.py + report_mapper).
    """
    try:
        ...
        page.screenshot(path=screenshot_path, full_page=False, animations="disabled")
