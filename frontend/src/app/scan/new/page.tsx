...
function priceHint(pkg: ReturnType<typeof getPackage>): string {
  ...
  return `Abo ab ${pkg.priceEur.toLocaleString('de-DE')} € / Jahr`;
}
    ...
    <div className="w-full max-w-[640px] mx-auto px-4 py-8 space-y-7">
      ...
      {stepId === 'upgrade' && pkgDef && (
        <section aria-labelledby="step-upgrade" className="space-y-5">
          ...
          <StateView
            variant="info"
            title={`${pkgDef.marketingName} freischalten`}
            description={
              pkgDef.sellability === 'sales_assisted'
                ? 'Dieses Paket richten wir gemeinsam mit dir ein — inklusive Compliance-Nachweis und Beratung. Fordere ein unverbindliches Angebot an, oder starte vorerst einen einmaligen Scan.'
