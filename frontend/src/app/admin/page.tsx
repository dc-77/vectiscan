...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [usersRes, statsRes, costsRes, reviewsRes, queueRes] = await Promise.all([
        listUsers(), getAdminStats(), getAiCosts(), getPendingReviews(), getReviewQueue(),
      ]);
      ...
      if (queueRes.success && queueRes.data) {
        setTargetReviewCount(queueRes.data.orders.length + queueRes.data.subscriptions.length);
      }
      setError(null);
    } catch {
  ...
  return (
    <main className="flex-1 px-4 py-8 md:px-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Link
          href="/admin/review"
          className={`block bg-[#1e293b] rounded-lg border p-4 transition-colors ${
            targetReviewCount > 0
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className={`text-sm font-medium ${targetReviewCount > 0 ? 'text-amber-400' : 'text-slate-300'}`}>
                Target-Review ({targetReviewCount} offen)
              </h2>
              <p className="text-xs text-slate-500 mt-1">
                Pre-Check-Ergebnisse prüfen, Targets freigeben und Scan-Autorisierungen hochladen.
              </p>
            </div>
