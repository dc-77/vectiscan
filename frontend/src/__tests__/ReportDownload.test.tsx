...
jest.mock('@/lib/api', () => ({
  getReportDownloadUrl: jest.fn((id: string) => `http://localhost:4000/api/orders/${id}/report`),
}));

describe('ReportDownload', () => {
  it('should show download button', () => {
    render(<ReportDownload orderId="123" domain="example.com" />);
  ...
  it('should link to report download URL', () => {
    ...
    expect(link.closest('a')).toHaveAttribute('href', 'http://localhost:4000/api/orders/123/report');
  });

  it('should show new scan button when onNewScan is provided', () => {
    const onNewScan = jest.fn();
    render(<ReportDownload orderId="123" domain="example.com" onNewScan={onNewScan} />);
