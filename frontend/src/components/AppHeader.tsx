...
export default function AppHeader() {
  ...
  const navItems = authed ? [
    { href: '/scan', label: 'Neuer Scan', highlight: true },
    { href: '/subscribe', label: 'Neues Abo', highlight: true },
    { href: '/dashboard', label: 'Dashboard' },
    { href: '/schedules', label: 'Zeitpläne' },
    { href: '/profile', label: 'Profil' },
  ] as const : [
    { href: '/pricing', label: 'Pakete & Preise' },
    { href: '/login', label: 'Anmelden', highlight: true },
  ] as const;
    ...
    <header className="h-10 shrink-0 flex items-center justify-between px-4"
      ...
      <Link href="/" className="flex items-center gap-2 hover:opacity-90 transition-opacity">
        <svg width="22" height="22" viewBox="0 0 44 44" fill="none" aria-hidden="true">
          <path d="M22 2L4 10v12c0 11 8 18 18 20 10-2 18-9 18-20V10L22 2z" fill="#1e293b" stroke="#2DD4BF" strokeWidth="2" />
          <line x1="12" y1="22" x2="32" y2="22" stroke="#2DD4BF" strokeWidth="1.5" opacity="0.8" />
          <line x1="22" y1="12" x2="22" y2="32" stroke="#2DD4BF" strokeWidth="1.5" opacity="0.8" />
          <circle cx="22" cy="22" r="3" fill="#2DD4BF" opacity="0.9" />
          <circle cx="22" cy="22" r="1.5" fill="#5EEAD4" />
        </svg>
        <span className="text-sm font-bold text-white tracking-tight">
          Vecti<span style={{ color: '#2DD4BF' }}>Scan</span>
        </span>
